...
data class SubLandFileSpec(
	...
	fun toSubLand() = SubLand(
		...
		새주소본번 = 새주소본번,
		새주소부번 = 새주소부번,
		부속대장구분코드 = 부속대장구분코드?.trim()?.ifEmpty { null },
		부속대장구분코드명 = 부속대장구분코드명?.trim()?.ifEmpty { null },
		부속시군구코드 = 부속시군구코드?.trim()?.ifEmpty { null },
		부속법정동코드 = 부속법정동코드?.trim()?.ifEmpty { null },
		부속대지구분코드 = 부속대지구분코드?.trim()?.ifEmpty { null },
		부속번 = 부속번?.trim()?.ifEmpty { null },
		부속지 = 부속지?.trim()?.ifEmpty { null },
		부속특수지명 = 부속특수지명?.trim()?.ifEmpty { null },
		부속블록 = 부속블록?.trim()?.ifEmpty { null },
		부속로트 = 부속로트?.trim()?.ifEmpty { null },
		부속기타지번명 = 부속기타지번명?.trim()?.ifEmpty { null },
